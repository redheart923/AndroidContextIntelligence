...
from workspace.pipeline import load_plan, run_java, run_kotlin, run_inheritance
...
''', encoding="utf-8")
    ...
    run_kotlin(load_plan(plan_path), db, ctags_dir)
    ...
    with sqlite3.connect(db) as connection:
        assert connection.execute(
            """
            SELECT line_end > line_start
            FROM node
            WHERE node_type='JAVA_METHOD'
              AND qualified_name='common.Base#bounded()'
            """
        ).fetchone() == (1,)
        assert connection.execute(
            """
            SELECT line_end > line_start
            FROM node
            WHERE node_type='KOTLIN_METHOD'
              AND display_name='boundedKotlin'
            """
        ).fetchone() == (1,)
